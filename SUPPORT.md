...
that keeps the exact source coordinates and bytes readable. The pinned Testlab
scenario sends that record to a distinct topic, independently observes both
records, and re-reads the retained source only after the destination terminal.
This remains configured qualification scope until archived evidence passes for
an exact client commit.
...
### Admin timestamp offset selection
...
from both `OffsetSpec::earliest` and `OffsetSpec::latest`. This remains
configured qualification scope until archived evidence passes for an exact
client commit.
...
`d8bc8044836067dba19348b8176b7e2c5ed6cc3b` defines the following gating cells.
