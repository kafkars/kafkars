...
and capacity limits as one `ProducerConfig`. The same complete policy can be
applied through `producer_delivery_timeout`, `producer_compression`,
...
### Direct-consumer configuration

`ClientBuilder::assigned_consumer_read_isolation`,
`assigned_consumer_fetch`, and `assigned_consumer_limits` fix the direct
consumer policy before the client host starts. The pinned Testlab protocol
requires one exact configured-client command containing every selected value;
a plain or producer-configured client with the same ID is not equivalent. Its
...
Each `ConsumerBuilder` registration fixes its originating client, member and
group identities, caller-ordered subscription, classic or KIP-848 protocol, and
complete optional policy. The pinned Testlab verifier requires that entire
registration once in the issued command and rejects a same-ID direct or Share
...
A successful creation also emits the exact group ID and caller-ordered
subscription read through `Consumer::group_id` and `Consumer::subscription` on
the returned handle; Testlab binds that observation to the creating command.
...
and group identities, caller-ordered subscription, optional rack, membership
and close deadlines, and complete optional acquisition policy. The pinned
Testlab verifier requires that entire registration once and rejects altered
fields, another same-ID consumer kind, or duplicate creation. Share assignment,
delivery, and acknowledgement remain proved by their public and independent
broker evidence.
A successful creation also emits the exact group ID, caller-ordered
subscription, and optional rack read through `ShareConsumer::group_id`,
`ShareConsumer::subscription`, and `ShareConsumer::rack` on the returned handle;
Testlab binds that observation to the creating command.

### Share-consumer receive commands

`ShareConsumer::recv` selects waiting acquisition into one retained linear
batch. The pinned Testlab protocol requires every receive command exactly once
in scenario order, retaining its consumer, batch identity, and complete
observation timeout. Expected producer identities and acquisition counts remain
...
### Admin consumer-group offset retention

The pinned Testlab lifecycle calls
`AlterConsumerGroupOffsetsBuilder::retention_time` with an explicit one-day
duration and preserves that exact selection in the adapter command. The same
cell requires caller-ordered public partition outcomes, distinct independently
corroborated baselines, and immediate independent post-state for both offsets.
It proves the requested mutation and retention selection, not eventual offset
...
must survive into each command, and every successful public description must
retain Kafka's authorization bitfield plus every applicable partition error,
leader, epoch, replica, ISR, and offline-replica getter. Explicit
`DescribeTopicPartitions` pages additionally retain eligible-leader and
last-known-eligible getters and must exactly reproduce the aggregate details.
Immediate metadata or pinned Kafka topic CLI observations independently
establish each topic identity and partition set without substituting for those
public-only details. This remains configured qualification scope until archived
evidence passes for an exact client commit.
...
authorization bitfield and complete applicable public partition topology,
while immediate metadata independently proves the exact error-free partition
set of each included expectation. A classic group
...
`1d1289290901abad4bfc28e2eab41481432ecf3c` defines the following gating cells.
...
Its controlled network-proxy cell now requires exact post-cut single-record,
complete-batch, transactional-batch, and classic/KIP-848
consume-transform-produce commands plus exact progress from the same directly
assigned, classic, KIP-848, and Share handles on independent public clients.
Proxy effect observations and client completions remain separate evidence.

Its broker-role cell also requires exact ordinary and transactional-batch
production plus directly assigned, classic-group, KIP-848 group, and Share
progress from each disrupted topic-partition after an independently observed
replacement election and before the original partition leader is restored.
Producer acknowledgement cannot satisfy the consumer contracts, and the
ordinary probe cannot satisfy the transaction contract.
...
leader movement, plus packaged directly assigned, classic-group, and KIP-848
group Fetch progress on each exact partition while its independently identified
original leader remains offline. This is source and qualification-scenario
coverage, not a production-support promise; any compatibility statement remains
limited to exact archived passing cells and separate support authorization.
