...
`47edc92118064389f9b7e5ed8ad08deb7a05b8e5` defines the following gating cells.
...
Its broker-role cell also requires an exact directly assigned Fetch from the
disrupted topic-partition after an independently observed replacement election
and before the original partition leader is restored. Producer acknowledgement
alone cannot satisfy that consumer contract.
...
leader movement, plus packaged directly assigned Fetch progress on the exact
partition while its independently identified original leader remains offline.
This is source and qualification-scenario coverage, not a production-support
promise; any compatibility statement remains limited to exact archived passing
cells and separate support authorization.
