...
| Admin | Broad concrete request-specific core, engine, and facade paths including exact-broker routes | Configured topic create/validate/batch/partition/delete/list lifecycles and failures, including caller-ordered name-based plural deletion with mixed outcomes and independent before/after metadata; singleton topic description plus caller-ordered detailed plural topic descriptions with mixed outcomes; topic configuration and caller-ordered offset queries; singleton and caller-ordered plural record deletion with explicit and high-watermark boundaries plus independent before/after ranges; cluster, consumer-group, and generic group discovery; singleton and caller-ordered plural active Share-group state and assignment descriptions, singleton and caller-ordered plural selected Share-group offset listings, Share-group offset alteration/deletion, and caller-ordered Share-group deletion with independent CLI queries; consumer-group offset list/alter/delete plus singleton and caller-ordered plural empty-group deletion with independent before/after group state; caller-ordered ACL lifecycle; named-user producer/consumer byte-rate quota replacement, description, and removal; and named-user SCRAM-SHA-256/512 credential upsert, description, and deletion with independent state queries |
...
`3e909dd34abed42fc2213a7c2cd4b63733ce4fc1` defines the following gating cells.
