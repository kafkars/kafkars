...
`456f9e30464c45359a4fbc8e8dfabbd578a03088` defines the following gating cells.
...
Its broker-role cell also requires exact directly assigned and classic-group
Fetch progress from each disrupted topic-partition after an independently
observed replacement election and before the original partition leader is
restored. Producer acknowledgement alone cannot satisfy those consumer
contracts.
...
leader movement, plus packaged directly assigned and classic-group Fetch
progress on each exact partition while its independently identified original
leader remains offline. This is source and qualification-scenario coverage, not
a production-support promise; any compatibility statement remains limited to
