...
These notes describe configured Testlab coverage, not passing qualification
results. See [support and compatibility](../SUPPORT.md) for the release boundary.
...
Record-fidelity coverage carries an explicit producer timestamp through
...
transactional batch whose complete record set stages and commits, exact classic
and KIP-848 transactional transforms whose outputs and checkpoints commit, plus
