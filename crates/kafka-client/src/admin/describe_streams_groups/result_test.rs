...
use crate::{BatchResult, ErrorKind, KafkaError, admin::StreamsGroupDescription};

use super::DescribeStreamsGroupsResult;

fn description(group_id: &str, epoch: i32) -> StreamsGroupDescription {
...
fn result_preserves_max_throttle_caller_order_and_partial_failures() {
    ...
    let result = DescribeStreamsGroupsResult::new(
...
fn into_groups_preserves_the_translation_boundary_entries() {
    ...
    let groups = DescribeStreamsGroupsResult::new(
        Duration::ZERO,
        BatchResult::new(vec![
