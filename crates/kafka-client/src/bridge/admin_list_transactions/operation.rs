//! Concrete observation of accepted or rejected transaction-listing work.
...
use crate::{ErrorKind, KafkaError, admin::ListTransactionsResult};
...
pub(crate) type AdminListTransactionsResult = Result<ListTransactionsResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminListTransactionsResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminListTransactions {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminListTransactions {
    ...
    pub(crate) fn wait(self) -> AdminListTransactionsResult {
        match self.inner {
    ...
    fn ready(result: AdminListTransactionsResult) -> Self {
        Self {
...
impl Future for AdminListTransactions {
    type Output = AdminListTransactionsResult;
...
impl fmt::Debug for AdminListTransactions {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminListTransactions")
...
fn already_observed() -> KafkaError {
    KafkaError::new(ErrorKind::State, "ListTransactions was already observed")
}
