...
use crate::{ErrorKind, KafkaError, admin::DescribeLogDirsResult};
...
pub(crate) type AdminDescribeLogDirsResult = Result<DescribeLogDirsResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeLogDirsResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeLogDirs {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeLogDirs {
    ...
    pub(crate) fn wait(self) -> AdminDescribeLogDirsResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeLogDirsResult) -> Self {
        Self {
...
impl Future for AdminDescribeLogDirs {
    type Output = AdminDescribeLogDirsResult;
...
impl fmt::Debug for AdminDescribeLogDirs {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeLogDirs")
...
fn already_observed() -> KafkaError {
    KafkaError::new(ErrorKind::State, "DescribeLogDirs was already observed")
}
