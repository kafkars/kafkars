//! Concrete observation of accepted or rejected token-renewal work.
...
use crate::{ErrorKind, KafkaError, admin::RenewDelegationTokenResult};
...
pub(crate) type AdminRenewDelegationTokenResult = Result<RenewDelegationTokenResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminRenewDelegationTokenResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminRenewDelegationToken {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminRenewDelegationToken {
    ...
    pub(crate) fn wait(self) -> AdminRenewDelegationTokenResult {
        match self.inner {
    ...
    fn ready(result: AdminRenewDelegationTokenResult) -> Self {
        Self {
...
impl Future for AdminRenewDelegationToken {
    type Output = AdminRenewDelegationTokenResult;
...
impl fmt::Debug for AdminRenewDelegationToken {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminRenewDelegationToken")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "RenewDelegationToken was already observed",
    )
}
