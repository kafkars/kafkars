...
use crate::{ErrorKind, KafkaError, admin::UpdateFeaturesResult};
...
pub(crate) type AdminUpdateFeaturesResult = Result<UpdateFeaturesResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminUpdateFeaturesResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminUpdateFeatures {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminUpdateFeatures {
    ...
    pub(crate) fn from_request_error(error: KafkaError) -> Self {
        Self::ready(Err(error))
    }

    pub(crate) fn wait(self) -> AdminUpdateFeaturesResult {
        match self.inner {
    ...
    fn ready(result: AdminUpdateFeaturesResult) -> Self {
        Self {
...
impl Future for AdminUpdateFeatures {
    type Output = AdminUpdateFeaturesResult;
...
impl fmt::Debug for AdminUpdateFeatures {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminUpdateFeatures")
...
fn already_observed() -> KafkaError {
    KafkaError::new(ErrorKind::State, "UpdateFeatures was already observed")
}
