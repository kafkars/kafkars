//! Concrete observation of accepted or rejected partition-transaction abort work.
...
pub(crate) type AdminAbortPartitionTransactionResult = Result<(), KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminAbortPartitionTransactionResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminAbortPartitionTransaction {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminAbortPartitionTransaction {
    ...
    pub(crate) fn wait(self) -> AdminAbortPartitionTransactionResult {
        match self.inner {
    ...
    fn ready(result: AdminAbortPartitionTransactionResult) -> Self {
        Self {
...
impl Future for AdminAbortPartitionTransaction {
    type Output = AdminAbortPartitionTransactionResult;
...
impl fmt::Debug for AdminAbortPartitionTransaction {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminAbortPartitionTransaction")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "AbortPartitionTransaction was already observed",
    )
}
