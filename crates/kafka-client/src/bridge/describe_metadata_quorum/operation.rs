...
use crate::{ErrorKind, KafkaError, admin::MetadataQuorumDescription};
...
pub(crate) type AdminDescribeMetadataQuorumResult = Result<MetadataQuorumDescription, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeMetadataQuorumResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeMetadataQuorum {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeMetadataQuorum {
    ...
    pub(crate) fn wait(self) -> AdminDescribeMetadataQuorumResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeMetadataQuorumResult) -> Self {
        Self {
...
impl Future for AdminDescribeMetadataQuorum {
    type Output = AdminDescribeMetadataQuorumResult;
...
impl fmt::Debug for AdminDescribeMetadataQuorum {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeMetadataQuorum")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "DescribeMetadataQuorum was already observed",
    )
}
