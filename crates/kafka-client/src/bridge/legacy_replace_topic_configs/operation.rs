//! Concrete observation of accepted or rejected legacy topic configuration replacement.
...
use crate::{ErrorKind, KafkaError, admin::LegacyReplaceTopicConfigsResult};
...
pub(crate) type AdminLegacyReplaceTopicConfigsResult =
    Result<LegacyReplaceTopicConfigsResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminLegacyReplaceTopicConfigsResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminLegacyReplaceTopicConfigs {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminLegacyReplaceTopicConfigs {
    ...
    pub(crate) fn wait(self) -> AdminLegacyReplaceTopicConfigsResult {
        match self.inner {
    ...
    fn ready(result: AdminLegacyReplaceTopicConfigsResult) -> Self {
        Self {
    ...
    #[cfg(test)]
    pub(super) fn ready_for_test(result: AdminLegacyReplaceTopicConfigsResult) -> Self {
        Self::ready(result)
    }
}

impl Future for AdminLegacyReplaceTopicConfigs {
    type Output = AdminLegacyReplaceTopicConfigsResult;
...
impl fmt::Debug for AdminLegacyReplaceTopicConfigs {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminLegacyReplaceTopicConfigs")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "LegacyReplaceTopicConfigs was already observed",
    )
}
