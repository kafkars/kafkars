//! Concrete observation of accepted or rejected token-description work.
...
use crate::{ErrorKind, KafkaError, admin::DescribeDelegationTokensResult};
...
pub(crate) type AdminDescribeDelegationTokensResult =
    Result<DescribeDelegationTokensResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeDelegationTokensResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeDelegationTokens {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeDelegationTokens {
    ...
    pub(crate) fn wait(self) -> AdminDescribeDelegationTokensResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeDelegationTokensResult) -> Self {
        Self {
...
impl Future for AdminDescribeDelegationTokens {
    type Output = AdminDescribeDelegationTokensResult;
...
impl fmt::Debug for AdminDescribeDelegationTokens {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeDelegationTokens")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "DescribeDelegationTokens was already observed",
    )
}
