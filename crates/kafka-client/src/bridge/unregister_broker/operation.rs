//! Concrete observation of accepted or rejected broker-unregistration work.
...
use crate::{ErrorKind, KafkaError, admin::UnregisterBrokerResult};
...
pub(crate) type AdminUnregisterBrokerResult = Result<UnregisterBrokerResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminUnregisterBrokerResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminUnregisterBroker {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminUnregisterBroker {
    ...
    pub(crate) fn wait(self) -> AdminUnregisterBrokerResult {
        match self.inner {
    ...
    fn ready(result: AdminUnregisterBrokerResult) -> Self {
        Self {
...
impl Future for AdminUnregisterBroker {
    type Output = AdminUnregisterBrokerResult;
...
impl fmt::Debug for AdminUnregisterBroker {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminUnregisterBroker")
...
fn already_observed() -> KafkaError {
    KafkaError::new(ErrorKind::State, "UnregisterBroker was already observed")
}
