//! Exhaustive stable translation of engine broker-unregistration outcomes.

use std::time::Duration;

use crate::{
    DeliveryStatus as PublicDeliveryStatus, ErrorKind, KafkaError,
    admin::UnregisterBrokerResult as PublicResult,
};
...
use super::{
    ...
    operation::AdminUnregisterBrokerResult,
};
...
pub(super) fn translate_admission_kind(kind: AdmissionErrorKind) -> KafkaError {
    ...
    KafkaError::new(
        public,
        format!("UnregisterBroker admission failed: {kind:?}"),
    )
...
pub(super) fn translate_accepted_fault(fault: AcceptedFaultKind) -> KafkaError {
    match fault {
        AcceptedFaultKind::Wake => KafkaError::new(
            ErrorKind::Internal,
            "UnregisterBroker was accepted but its host wake failed",
        ),
        AcceptedFaultKind::HostInvariant => KafkaError::new(
            ErrorKind::Internal,
            "UnregisterBroker was accepted but its host reported an invariant failure",
        ),
    }
}
...
) -> AdminUnregisterBrokerResult {
    match result {
        Ok(Outcome::Unregistered(result)) => Ok(translate_success(result)),
...
fn translate_success(result: EngineResult) -> PublicResult {
    translate_success_parts(result.into_parts())
}
...
pub(super) fn translate_broker_error_parts(
    ...
    let diagnostic = message.map_or_else(
        || {
            format!(
                "Kafka rejected UnregisterBroker with broker code {code} after \
...
pub(super) fn translate_failure_parts(kind: FailureKind, delivery: DeliveryStatus) -> KafkaError {
    ...
    KafkaError::new(public, format!("UnregisterBroker failed: {kind:?}"))
