//! Concrete observation of accepted or rejected delegation-token creation.
...
use crate::{ErrorKind, KafkaError, admin::CreateDelegationTokenResult};
...
pub(crate) type AdminCreateDelegationTokenResult = Result<CreateDelegationTokenResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminCreateDelegationTokenResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminCreateDelegationToken {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminCreateDelegationToken {
    ...
    pub(crate) fn wait(self) -> AdminCreateDelegationTokenResult {
        match self.inner {
    ...
    fn ready(result: AdminCreateDelegationTokenResult) -> Self {
        Self {
...
impl Future for AdminCreateDelegationToken {
    type Output = AdminCreateDelegationTokenResult;
...
impl fmt::Debug for AdminCreateDelegationToken {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminCreateDelegationToken")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "CreateDelegationToken was already observed",
    )
}
