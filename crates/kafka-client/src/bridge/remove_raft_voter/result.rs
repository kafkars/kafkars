//! Exhaustive stable translation of engine-owned `RemoveRaftVoter` outcomes.

use std::time::Duration;

use crate::{
    DeliveryStatus as PublicDeliveryStatus, ErrorKind, KafkaError,
    admin::RemoveRaftVoterResult as PublicResult,
};
...
use super::{
    ...
    operation::AdminRemoveRaftVoterResult,
};
...
pub(super) fn translate_admission_kind(kind: AdmissionErrorKind) -> KafkaError {
    ...
    KafkaError::new(
        public,
        format!("RemoveRaftVoter admission failed: {kind:?}"),
    )
...
pub(super) fn translate_accepted_fault(fault: AcceptedFaultKind) -> KafkaError {
    match fault {
        AcceptedFaultKind::Wake => KafkaError::new(
            ErrorKind::Internal,
            "RemoveRaftVoter was accepted but its host wake failed",
        ),
        AcceptedFaultKind::HostInvariant => KafkaError::new(
            ErrorKind::Internal,
            "RemoveRaftVoter was accepted but its host reported an invariant failure",
        ),
    }
}
...
) -> AdminRemoveRaftVoterResult {
    match result {
        Ok(Outcome::Removed(result)) => Ok(translate_success(result)),
...
pub(super) fn translate_broker_error_parts(
    ...
    let diagnostic = message.map_or_else(
        || {
            format!(
                "Kafka rejected RemoveRaftVoter with broker code {code} after \
...
pub(super) fn translate_failure_parts(kind: FailureKind, delivery: DeliveryStatus) -> KafkaError {
    ...
    KafkaError::new(public, format!("RemoveRaftVoter failed: {kind:?}"))
