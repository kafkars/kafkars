...
use crate::{ErrorKind, KafkaError, admin::AlterClientQuotasResult};
...
pub(crate) type AdminAlterClientQuotasResult = Result<AlterClientQuotasResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminAlterClientQuotasResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminAlterClientQuotas {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminAlterClientQuotas {
    ...
    pub(crate) fn wait(self) -> AdminAlterClientQuotasResult {
        match self.inner {
    ...
    fn ready(result: AdminAlterClientQuotasResult) -> Self {
        Self {
...
impl Future for AdminAlterClientQuotas {
    type Output = AdminAlterClientQuotasResult;
...
impl fmt::Debug for AdminAlterClientQuotas {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminAlterClientQuotas")
...
fn already_observed() -> KafkaError {
    KafkaError::new(ErrorKind::State, "AlterClientQuotas was already observed")
}
