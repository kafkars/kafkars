//! Concrete observation of accepted or rejected SCRAM description work.
...
use crate::{ErrorKind, KafkaError, admin::DescribeUserScramCredentialsResult};
...
pub(crate) type AdminDescribeUserScramCredentialsResult =
    Result<DescribeUserScramCredentialsResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeUserScramCredentialsResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeUserScramCredentials {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeUserScramCredentials {
    ...
    pub(crate) fn wait(self) -> AdminDescribeUserScramCredentialsResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeUserScramCredentialsResult) -> Self {
        Self {
...
impl Future for AdminDescribeUserScramCredentials {
    type Output = AdminDescribeUserScramCredentialsResult;
...
impl fmt::Debug for AdminDescribeUserScramCredentials {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeUserScramCredentials")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "DescribeUserScramCredentials was already observed",
    )
}
