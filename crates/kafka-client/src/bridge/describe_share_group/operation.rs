...
use crate::{ErrorKind, KafkaError, admin::DescribeShareGroupResult};
...
pub(crate) type AdminDescribeShareGroupResult = Result<DescribeShareGroupResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeShareGroupResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeShareGroup {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeShareGroup {
    ...
    pub(crate) fn wait(self) -> AdminDescribeShareGroupResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeShareGroupResult) -> Self {
        Self {
    ...
    #[cfg(test)]
    pub(super) fn ready_for_test(result: AdminDescribeShareGroupResult) -> Self {
        Self::ready(result)
    }
}

impl Future for AdminDescribeShareGroup {
    type Output = AdminDescribeShareGroupResult;
...
impl fmt::Debug for AdminDescribeShareGroup {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeShareGroup")
...
fn already_observed() -> KafkaError {
    KafkaError::new(ErrorKind::State, "DescribeShareGroup was already observed")
}
