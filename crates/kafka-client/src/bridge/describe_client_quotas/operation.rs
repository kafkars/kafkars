//! Concrete observation of accepted or rejected DescribeClientQuotas work.
...
use crate::{ErrorKind, KafkaError, admin::DescribeClientQuotasResult};
...
pub(crate) type AdminDescribeClientQuotasResult = Result<DescribeClientQuotasResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeClientQuotasResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeClientQuotas {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeClientQuotas {
    ...
    pub(crate) fn wait(self) -> AdminDescribeClientQuotasResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeClientQuotasResult) -> Self {
        Self {
...
impl Future for AdminDescribeClientQuotas {
    type Output = AdminDescribeClientQuotasResult;
...
impl fmt::Debug for AdminDescribeClientQuotas {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeClientQuotas")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "DescribeClientQuotas was already observed",
    )
}
