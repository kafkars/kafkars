//! Concrete observation of accepted or rejected `ShareGroup` offset deletion.
...
use crate::{ErrorKind, KafkaError, admin::DeleteShareGroupOffsetsResult};
...
pub(crate) type AdminDeleteShareGroupOffsetsResult =
    Result<DeleteShareGroupOffsetsResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDeleteShareGroupOffsetsResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDeleteShareGroupOffsets {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDeleteShareGroupOffsets {
    ...
    pub(crate) fn wait(self) -> AdminDeleteShareGroupOffsetsResult {
        match self.inner {
    ...
    fn ready(result: AdminDeleteShareGroupOffsetsResult) -> Self {
        Self {
...
impl Future for AdminDeleteShareGroupOffsets {
    type Output = AdminDeleteShareGroupOffsetsResult;
...
impl fmt::Debug for AdminDeleteShareGroupOffsets {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDeleteShareGroupOffsets")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "DeleteShareGroupOffsets was already observed",
    )
}
