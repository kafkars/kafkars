//! Public shape checks for inert streams-group deletion options.

use std::time::Duration;

use super::DeleteStreamsGroupsBuilder;
use crate::{Client, DeliveryStatus, ErrorKind};

#[test]
fn builder_is_thread_transferable() {
    fn assert_send_sync<T: Send + Sync>() {}
    assert_send_sync::<DeleteStreamsGroupsBuilder>();
}
...
fn public_handle_keeps_zero_deadline_inert_until_submit() {
    ...
    let error = client
        .admin()
        .delete_streams_groups(["streams-workers"])
