//! Ready and runtime-neutral group-offset deletion observation scenarios.
...
fn wait_and_future_share_one_concrete_result_shape() {
    let result =
        DeleteConsumerGroupOffsetsResult::new(Duration::ZERO, BatchResult::new(Vec::new()));
    let waited = AdminDeleteConsumerGroupOffsets::ready_for_test(Ok(result))
        .wait()
        .unwrap_or_else(|error| panic!("ready wait should succeed: {error}"));
    assert!(waited.offsets().entries().is_empty());

    let mut operation = AdminDeleteConsumerGroupOffsets::ready_for_test(Err(KafkaError::new(
        ErrorKind::Backpressure,
        "group-offset deletion capacity is full",
    )));
...
fn private_operation_is_a_send_future_without_a_runtime() {
    ...
    assert_future::<AdminDeleteConsumerGroupOffsets>();
}
