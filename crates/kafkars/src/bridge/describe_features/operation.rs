//! Concrete observation of accepted or rejected Kafka feature discovery.
...
use crate::{ErrorKind, KafkaError, admin::DescribeFeaturesResult};
...
pub(crate) type AdminDescribeFeaturesResult = Result<DescribeFeaturesResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeFeaturesResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeFeatures {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeFeatures {
    ...
    pub(crate) fn wait(self) -> AdminDescribeFeaturesResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeFeaturesResult) -> Self {
        Self {
...
impl Future for AdminDescribeFeatures {
    type Output = AdminDescribeFeaturesResult;
...
impl fmt::Debug for AdminDescribeFeatures {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeFeatures")
...
fn already_observed() -> KafkaError {
    KafkaError::new(ErrorKind::State, "DescribeFeatures was already observed")
}
