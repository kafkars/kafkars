//! Concrete observation of accepted or rejected client-metrics resource work.
...
use crate::{ErrorKind, KafkaError, admin::ListClientMetricsResourcesResult};
...
pub(crate) type AdminListClientMetricsResourcesResult =
    Result<ListClientMetricsResourcesResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminListClientMetricsResourcesResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminListClientMetricsResources {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminListClientMetricsResources {
    ...
    pub(crate) fn wait(self) -> AdminListClientMetricsResourcesResult {
        match self.inner {
    ...
    fn ready(result: AdminListClientMetricsResourcesResult) -> Self {
        Self {
...
impl Future for AdminListClientMetricsResources {
    type Output = AdminListClientMetricsResourcesResult;
...
impl fmt::Debug for AdminListClientMetricsResources {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminListClientMetricsResources")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "ListClientMetricsResources was already observed",
    )
}
