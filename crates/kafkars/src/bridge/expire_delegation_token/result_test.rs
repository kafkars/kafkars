//! Expiration error and delivery translation evidence.
...
#[test]
fn exact_signed_broker_code_and_throttle_are_lossless() {
    let error = translate_broker_error_parts(40, -30_000);
    assert_eq!(error.kind(), ErrorKind::Broker);
    assert_eq!(error.broker_code(), Some(-30_000));
