//! Concrete observation of accepted or rejected token-expiration work.
...
use crate::{ErrorKind, KafkaError, admin::ExpireDelegationTokenResult};
...
pub(crate) type AdminExpireDelegationTokenResult = Result<ExpireDelegationTokenResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminExpireDelegationTokenResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminExpireDelegationToken {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminExpireDelegationToken {
    ...
    pub(crate) fn wait(self) -> AdminExpireDelegationTokenResult {
        match self.inner {
    ...
    fn ready(result: AdminExpireDelegationTokenResult) -> Self {
        Self {
...
impl Future for AdminExpireDelegationToken {
    type Output = AdminExpireDelegationTokenResult;
...
impl fmt::Debug for AdminExpireDelegationToken {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminExpireDelegationToken")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "ExpireDelegationToken was already observed",
    )
}
