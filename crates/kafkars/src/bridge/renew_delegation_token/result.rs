//! Exhaustive stable translation of delegation-token renewal outcomes.

use std::time::Duration;

use crate::{
    DeliveryStatus as PublicDeliveryStatus, ErrorKind, KafkaError,
    admin::RenewDelegationTokenResult as PublicResult,
};
...
use super::{
    ...
    operation::AdminRenewDelegationTokenResult,
};
...
pub(super) fn translate_admission_kind(kind: AdmissionErrorKind) -> KafkaError {
    ...
    KafkaError::new(
        public,
        format!("RenewDelegationToken admission failed: {kind:?}"),
    )
...
pub(super) fn translate_accepted_fault(fault: AcceptedFaultKind) -> KafkaError {
    match fault {
        AcceptedFaultKind::Wake => KafkaError::new(
            ErrorKind::Internal,
            "RenewDelegationToken was accepted but its host wake failed",
        ),
        AcceptedFaultKind::HostInvariant => KafkaError::new(
            ErrorKind::Internal,
            "RenewDelegationToken was accepted but its host reported an invariant failure",
        ),
    }
}
...
) -> AdminRenewDelegationTokenResult {
    match result {
        Ok(Outcome::Renewed(result)) => Ok(translate_success(result)),
...
pub(super) fn translate_broker_error_parts(throttle_time_ms: u32, code: i16) -> KafkaError {
    KafkaError::new(
        ErrorKind::Broker,
        format!(
            "Kafka rejected RenewDelegationToken with broker code {code} after \
...
pub(super) fn translate_failure_parts(kind: FailureKind, delivery: DeliveryStatus) -> KafkaError {
    KafkaError::new(
        kind_to_error(kind),
        format!("RenewDelegationToken failed: {kind:?}"),
    )
