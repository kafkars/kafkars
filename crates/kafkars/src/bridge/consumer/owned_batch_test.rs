...
use super::{
    ...
    owned_batch::record_from_shared_delivery_parts,
};
...
fn owned_bridge_capabilities_are_send_and_consume_linearly() {
    ...
    fn require_record_transfer(
        _transfer: fn(AssignedConsumerOwnedRecord, std::sync::Arc<str>) -> crate::Record,
    ) {
    }
    ...
    require_record_transfer(AssignedConsumerOwnedRecord::into_record);
}

#[test]
fn owned_conversion_and_rejection_preserve_pointers_semantics_and_source_lease() {
    ...
    let record = record_from_shared_delivery_parts(
