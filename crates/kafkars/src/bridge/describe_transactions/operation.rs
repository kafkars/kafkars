//! Concrete observation of accepted or rejected Admin `DescribeTransactions` work.
...
use crate::{ErrorKind, KafkaError, admin::DescribeTransactionsResult};
...
pub(crate) type AdminDescribeTransactionsResult = Result<DescribeTransactionsResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeTransactionsResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeTransactions {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeTransactions {
    ...
    pub(crate) fn wait(self) -> AdminDescribeTransactionsResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeTransactionsResult) -> Self {
        Self {
...
impl Future for AdminDescribeTransactions {
    type Output = AdminDescribeTransactionsResult;
...
impl fmt::Debug for AdminDescribeTransactions {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeTransactions")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "DescribeTransactions was already observed",
    )
}
