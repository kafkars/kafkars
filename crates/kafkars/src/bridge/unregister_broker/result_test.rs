//! Broker-unregistration result, category, diagnostic, and delivery tests.
...
#[test]
fn success_keeps_kafka_throttle_observation() {
    let result = translate_success_parts(41);

    assert_eq!(result.throttle_time(), std::time::Duration::from_millis(41));
}

#[test]
fn broker_rejection_keeps_exact_signed_code_diagnostic_and_delivery() {
    let error = translate_broker_error_parts(37, -731, Some("bounded"), true);
    ...
    assert!(error.to_string().contains("37 ms throttle"));
