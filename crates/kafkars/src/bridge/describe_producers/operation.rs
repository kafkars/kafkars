//! Concrete observation of accepted or rejected Admin `DescribeProducers` work.
...
use crate::{ErrorKind, KafkaError, admin::DescribeProducersResult};
...
pub(crate) type AdminDescribeProducersResult = Result<DescribeProducersResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminDescribeProducersResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminDescribeProducers {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminDescribeProducers {
    ...
    pub(crate) fn wait(self) -> AdminDescribeProducersResult {
        match self.inner {
    ...
    fn ready(result: AdminDescribeProducersResult) -> Self {
        Self {
...
impl Future for AdminDescribeProducers {
    type Output = AdminDescribeProducersResult;
...
impl fmt::Debug for AdminDescribeProducers {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminDescribeProducers")
...
fn already_observed() -> KafkaError {
    KafkaError::new(ErrorKind::State, "DescribeProducers was already observed")
}
