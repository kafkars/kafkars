...
use crate::{ErrorKind, KafkaError, admin::ListShareGroupOffsetsResult};
...
pub(crate) type AdminListShareGroupOffsetsResult = Result<ListShareGroupOffsetsResult, KafkaError>;

enum Inner {
    Accepted(EngineObserver),
    Ready(Option<AdminListShareGroupOffsetsResult>),
}

/// Private named observation shared by async and blocking facade paths.
#[must_use = "dropping abandons observation without cancelling accepted admin work"]
pub(crate) struct AdminListShareGroupOffsets {
    inner: Inner,
    accepted_diagnostic: Option<KafkaError>,
}

impl AdminListShareGroupOffsets {
    ...
    pub(crate) fn wait(self) -> AdminListShareGroupOffsetsResult {
        match self.inner {
    ...
    fn ready(result: AdminListShareGroupOffsetsResult) -> Self {
        Self {
    ...
    #[cfg(test)]
    pub(super) fn ready_for_test(result: AdminListShareGroupOffsetsResult) -> Self {
        Self::ready(result)
    }
}

impl Future for AdminListShareGroupOffsets {
    type Output = AdminListShareGroupOffsetsResult;
...
impl fmt::Debug for AdminListShareGroupOffsets {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("AdminListShareGroupOffsets")
...
fn already_observed() -> KafkaError {
    KafkaError::new(
        ErrorKind::State,
        "ListShareGroupOffsets was already observed",
    )
}
