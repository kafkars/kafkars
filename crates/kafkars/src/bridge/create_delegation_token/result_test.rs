//! Delegation-token creation error, delivery, and secret-safety translation.
...
fn admission_categories_are_exhaustive_and_definitely_unsent() {
    for (kind, expected) in [
        ...
        (AdmissionErrorKind::HostInvariant, ErrorKind::Internal),
...
#[test]
fn exact_signed_broker_code_and_throttle_are_diagnostic_without_secret_bytes() {
    let error = translate_broker_error_parts(41, -32_000);
    ...
    assert!(diagnostic.contains("41"));
    assert!(diagnostic.contains("-32000"));
    assert!(!diagnostic.contains("hmac"));
}
