//! Allocation-free normalization of one API-key 40 scalar terminal.

use kafka_wire::ExpireDelegationTokenResponse;

use super::{
    NormalizedExpireDelegationTokenResponse,
    retention::{EXPIRE_DELEGATION_TOKEN_MAX_RETAINED_BYTES, response_charge},
};
...
pub(crate) enum ExpireDelegationTokenResponseFailure {
...
pub(crate) fn normalize_expire_delegation_token_response(
    selected_version: Option<i16>,
    response: &ExpireDelegationTokenResponse,
    retained_limit: usize,
) -> Result<NormalizedExpireDelegationTokenResponse, ExpireDelegationTokenResponseFailure> {
    let selected_version =
        selected_version.ok_or(ExpireDelegationTokenResponseFailure::MissingSelectedVersion)?;
    ...
    if required > effective_limit {
        return Err(ExpireDelegationTokenResponseFailure::RetainedBytes {
            required,
            limit: effective_limit,
        });
    }
    Ok(NormalizedExpireDelegationTokenResponse::new(
        throttle_time_ms,
        response.error_code,
        expiry_timestamp_ms,
        required,
