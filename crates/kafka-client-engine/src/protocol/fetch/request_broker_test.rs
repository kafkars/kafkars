...
use super::{
    BrokerFetchPartition, FetchRequestSettings, FetchSessionRequest, ForgottenFetchPartition,
    broker_fetch_request, fetch_session_close_request,
};
...
fn one_broker_request_aggregates_topics_and_partitions_in_input_order() {
    let request = broker_fetch_request(
        &[
            BrokerFetchPartition::new("alpha", 1, 11),
            BrokerFetchPartition::new("beta", 2, 22),
            BrokerFetchPartition::new("alpha", 3, 33),
        ],
        &[],
        settings(),
    ...
    assert!(request.encoded_len(ApiVersion::new(7)).is_ok());
}
...
fn incremental_request_groups_forgotten_partitions_by_topic() {
    ...
    let request = broker_fetch_request(
        &[BrokerFetchPartition::new("active", 0, 10)],
        &[
            ForgottenFetchPartition::new("alpha", 1),
            ForgottenFetchPartition::new("beta", 2),
            ForgottenFetchPartition::new("alpha", 3),
        ],
        settings(),
        session,
    )
