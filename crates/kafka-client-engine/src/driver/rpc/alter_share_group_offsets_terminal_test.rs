...
use super::alter_share_group_offsets_terminal::{
    AlterShareGroupOffsetsDriverFailureKind, AlterShareGroupOffsetsTerminalFact,
    RecoveredAlterShareGroupOffsetsCall, retain_alter_share_group_offsets_terminal,
};
...
fn response_fact_borrows_exact_v0_and_generated_response() {
    ...
    let terminal =
        retain_alter_share_group_offsets_terminal(Some(ApiVersion::new(0)), Ok(response), None);
    let AlterShareGroupOffsetsTerminalFact::Response {
        selected_version,
        response,
    } = terminal.fact()
    else {
...
fn failures_preserve_delivery_certainty_and_stable_classification() {
    ...
    for (error, expected_kind, expected_delivery) in cases {
        let terminal = retain_alter_share_group_offsets_terminal(None, Err(error), None);
        let AlterShareGroupOffsetsTerminalFact::Failed { kind, delivery } = terminal.fact() else {
...
fn shutdown_recovery_token_seals_linearly() {
    RecoveredAlterShareGroupOffsetsCall::new().seal();
}
