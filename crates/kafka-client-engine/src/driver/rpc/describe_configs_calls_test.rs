//! Bounded tracked-call ownership scenarios for `DescribeConfigs`.
...
use super::describe_configs_calls::DescribeConfigsCalls;

#[test]
fn accepted_call_occupies_the_only_slot_until_driver_shutdown() {
    let mut owner = owner();
    let mut calls = DescribeConfigsCalls::new(1);
    let permit = calls
        .try_reserve()
    ...
    permit
        ...
        .unwrap_or_else(|error| panic!("submit tracked DescribeConfigs call: {error:?}"));
...
fn zero_capacity_rejects_before_submission() {
    let mut calls = DescribeConfigsCalls::new(0);
    assert!(calls.try_reserve().is_none());
}
