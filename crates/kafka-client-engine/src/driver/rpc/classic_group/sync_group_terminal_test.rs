//! Raw Sync terminal retention scenarios.
...
use super::sync_group_terminal::{SyncGroupCallKey, retain_sync_group_terminal};
...
fn raw_success_preserves_group_cycle_deadline_version_and_response() {
    ...
    let terminal = retain_sync_group_terminal(key, Some(ApiVersion::new(2)), Ok(response));
...
fn raw_driver_failure_remains_uninterpreted() {
    let key = key(1);
    let terminal = retain_sync_group_terminal(key, None, Err(RequestError::RouteUnavailable));
...
pub(super) fn key(group: u64) -> SyncGroupCallKey {
    key_with_deadline(group, deadline())
}

pub(super) fn key_with_deadline(group: u64, deadline: OperationDeadline) -> SyncGroupCallKey {
    let group_id = GroupId::try_from_raw(group)
        .unwrap_or_else(|| panic!("test group identity must be nonzero"));
    SyncGroupCallKey::new(group_id, MembershipCycle::initial(), deadline)
}
