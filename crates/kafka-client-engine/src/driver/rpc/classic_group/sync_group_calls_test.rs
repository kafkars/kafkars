//! Bounded Sync call registry scenarios.

use kafka_driver::CompletionError;
use kafka_wire::SyncGroupResponse;

use super::{
    sync_group_calls::{SyncGroupCallReservationError, TrackedSyncGroupCalls},
    sync_group_settlement::SyncGroupPoll,
...
fn retained_terminal_consumes_the_exact_bounded_slot() {
    ...
    let first_key = key(1);
    assert!(calls.try_reserve_sync_group(first_key, "group-a").is_ok());

    calls.install_terminal_for_test(first_key, Some(2), Ok(SyncGroupResponse::default()));

    assert_eq!(calls.retained_sync_group_count(), 1);
    assert_eq!(
        calls.try_reserve_sync_group(first_key, "group-a").err(),
        Some(SyncGroupCallReservationError::Duplicate { key: first_key })
    );
    assert_eq!(
        calls.try_reserve_sync_group(key(2), "group-b").err(),
        Some(SyncGroupCallReservationError::Capacity { limit: 1 })
    );
    assert_eq!(
        calls.poll_sync_group(),
        Ok(SyncGroupPoll::TerminalReady { key: first_key })
    );
}
...
fn equal_cycle_and_deadline_for_another_group_is_not_a_duplicate() {
    ...
    let mut calls = TrackedSyncGroupCalls::new(2);
    calls.install_terminal_for_test(first, Some(2), Ok(SyncGroupResponse::default()));

    assert!(calls.try_reserve_sync_group(second, "group-b").is_ok());
}
...
fn same_group_and_cycle_with_a_different_deadline_is_a_duplicate() {
    ...
    assert_eq!(
        calls.try_reserve_sync_group(second, "group-a").err(),
        Some(SyncGroupCallReservationError::Duplicate { key: second })
    );
}
...
fn completion_fault_remains_retained_and_observable() {
    ...
    let observation = match calls.poll_sync_group() {
