...
use kafka_client_core::{DeliveryStatus, ExpireDelegationTokenHmac, ExpireDelegationTokenPlan};
...
fn failures_preserve_delivery_certainty_and_stable_classification() {
    ...
    for (error, expected_kind, expected_delivery) in cases {
        let terminal = retain_expire_delegation_token_terminal(None, Err(error), None, plan());
...
fn shutdown_recovery_token_seals_linearly() {
    RecoveredExpireDelegationTokenCall::for_test(plan()).seal();
}

fn plan() -> ExpireDelegationTokenPlan {
    let hmac = ExpireDelegationTokenHmac::new(vec![0xA5, 0x5A])
