...
use crate::consumer::assigned_event::AssignedConsumerEventStoreError;

use super::super::classic_group_position::{
    ClassicGroupPositionActivationError, ClassicGroupPositionCompleted,
};
use super::model::ClassicGroupFetchPreflightError;
...
pub(in crate::consumer::group) struct ClassicGroupFetchBinding(GroupPositionFence, AssignmentEpoch);
...
/// Installed assignment binding whose ordered effects moved into the owner FIFO.
#[must_use = "an active group Fetch binding fences all later execution"]
pub(in crate::consumer::group) struct ClassicGroupFetchActivation(ClassicGroupFetchBinding);
...
impl ClassicGroupFetchActivation {
    pub(super) const fn new(binding: ClassicGroupFetchBinding) -> Self {
        Self(binding)
    }
...
pub(in crate::consumer::group) enum ClassicGroupFetchActivationFailureKind {
    ...
    Preflight(ClassicGroupFetchPreflightError),
...
enum ClassicGroupFetchActivationFailureSource {
    ...
    Position(ClassicGroupPositionActivationError),
    Preflight {
        error: ClassicGroupFetchPreflightError,
...
impl ClassicGroupFetchActivationFailure {
    ...
    pub(super) const fn preflight(
        ...
        Self {
            completed,
            source: ClassicGroupFetchActivationFailureSource::Preflight { error, input },
        }
    }
    ...
    pub(super) const fn kind(&self) -> ClassicGroupFetchActivationFailureKind {
        match &self.source {
            ...
            ClassicGroupFetchActivationFailureSource::Preflight { error, .. } => {
                ClassicGroupFetchActivationFailureKind::Preflight(*error)
            }
    ...
    pub(super) const fn rejected_input(&self) -> Option<&InstallResolvedAssignment> {
        match &self.source {
            ClassicGroupFetchActivationFailureSource::Preflight { input, .. } => Some(input),
