...
use super::{
    consumer_group_close::fail_consumer_group_leave,
...
fn prepare_steady_request(
    ...
    match (prepared.assignment_generation(), reportable) {
        ...
        (None, None)
            if execution.machine().pending_assignment().is_some()
                || execution.machine().phase()
                    == kafka_client_core::ConsumerGroupHeartbeatPhase::Heartbeating => {}
        _ => return Err(()),
    }
    ...
    match (prepared.assignment_generation(), reportable) {
        (Some(generation), Some(assignment))
            if assignment.member_id() == member_id
                && assignment.assignment_generation() == generation => {}
        (None, None)
            if execution.machine().phase()
                == kafka_client_core::ConsumerGroupHeartbeatPhase::Leaving
