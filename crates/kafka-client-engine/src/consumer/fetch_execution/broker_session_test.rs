...
fn established(effects: &[AssignedConsumerEffect], broker: BrokerId) -> BrokerFetchSessions {
    let mut sessions = sessions();
    let plan = sessions
        .try_begin(
            broker,
...
fn member(effect: AssignedConsumerEffect, topic: &str) -> BrokerSessionMember {
    BrokerSessionMember::new(fetch_fence(effect).position(), Arc::from(topic))
}

fn incremental(id: i32, epoch: i32) -> FetchSessionRequest {
