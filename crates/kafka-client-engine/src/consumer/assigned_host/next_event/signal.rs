//! One generation-fenced waiter shared by task and blocking event observation.
...
/// Exact generation of one active event registration.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct AssignedConsumerEventRegistration(u64);

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum AssignedConsumerEventWait {
    Change,
    Unlock,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum AssignedConsumerEventSignalError {
    GenerationExhausted,
    Stale,
}

struct Registration {
    id: AssignedConsumerEventRegistration,
...
/// Capacity-one notification state; it owns no event, Fetch work, or deadline.
pub(crate) struct AssignedConsumerEventSignal {
    state: Mutex<SignalState>,
    changed: Condvar,
}

impl AssignedConsumerEventSignal {
    ...
    pub(crate) fn arm_task(
        &self,
        current: Option<AssignedConsumerEventRegistration>,
        wait: AssignedConsumerEventWait,
        waker: &Waker,
    ) -> Result<AssignedConsumerEventRegistration, AssignedConsumerEventSignalError> {
    ...
    pub(super) fn arm_blocking(
        ...
        self.arm(current, AssignedConsumerEventWait::Change, None, true)
    }

    pub(crate) fn prepare_notification(&self, wake: AssignedConsumerEventWait) -> bool {
        ...
        if registration.notified
            ...
            || (wake == AssignedConsumerEventWait::Unlock
                && registration.wait != AssignedConsumerEventWait::Unlock)
        {
            return false;
        }
    ...
    pub(super) fn wait(
        &self,
        id: AssignedConsumerEventRegistration,
    ) -> Result<(), AssignedConsumerEventSignalError> {
        let mut state = self.lock();
        loop {
            let Some(registration) = state
                ...
                return Err(AssignedConsumerEventSignalError::Stale);
            };
            if registration.notified {
    ...
    pub(crate) fn cancel(&self, id: AssignedConsumerEventRegistration) {
        let mut state = self.lock();
        if state
            .registration
            .as_ref()
    ...
    fn arm(
        ...
        let id = if let Some(id) = current {
            ...
            let generation = state
                .next_generation
                .ok_or(AssignedConsumerEventSignalError::GenerationExhausted)?;
            state.next_generation = generation.checked_add(1);
            AssignedConsumerEventRegistration(generation)
        };
        match state.registration.as_mut() {
            ...
            Some(_) => return Err(AssignedConsumerEventSignalError::Stale),
            None => {
