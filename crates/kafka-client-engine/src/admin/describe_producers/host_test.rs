...
use kafka_client_core::{AdminDescribeProducerTarget, AdminDescribeProducersPlan};
...
use super::{
    ...
    AdminDescribeProducersFailureKind, AdminDescribeProducersOutcome, AdminDescribeProducersTurn,
...
fn admission_reserves_terminal_and_full_envelope_before_first_target() {
    ...
    let (_operation_id, submitted_deadline, target, broker_id) = submission.into_parts();
...
fn untouched_and_handed_off_recovery_preserve_delivery_certainty() {
    for handed_off in [false, true] {
        ...
        let admission = host
            ...
            .unwrap_or_else(|error| panic!("admit DescribeProducers: {error:?}"));
        if handed_off {
            let AdminDescribeProducersTurn::Submit(_) = host
                .turn(capture.now())
