//! Nonblocking offset-alteration shard admission and close scenarios.
...
use super::{
    AlterConsumerGroupOffsetsAdmissionErrorKind, AlterConsumerGroupOffsetsShardOwner,
    AlterConsumerGroupOffsetsShardWake, AlterConsumerGroupOffsetsShardWakeError,
};

struct NoopWake;

impl AlterConsumerGroupOffsetsShardWake for NoopWake {
...
fn closed_port_rejects_before_completion_or_bytes_are_reserved() {
    let (host, notifier) = crate::admin::test_support::alter_consumer_group_offsets_host();
    let owner = AlterConsumerGroupOffsetsShardOwner::new(host, Arc::new(NoopWake));
    let port = owner.admission_port();
    port.close_admission()
        .unwrap_or_else(|error| panic!("close offset-alteration admission: {error:?}"));
    assert!(matches!(
        port.try_admit(Moment::from_tick(1), deadline(), plan()),
        Err(AlterConsumerGroupOffsetsAdmissionErrorKind::Closed)
    ));
...
fn wake_error_retains_its_io_cause_without_becoming_policy() {
    let error = AlterConsumerGroupOffsetsShardWakeError::from_io(std::io::Error::other("closed"));
