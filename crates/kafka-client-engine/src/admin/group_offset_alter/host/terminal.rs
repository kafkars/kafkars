...
use kafka_client_core::AlterConsumerGroupOffsetsEffect;

use crate::completion::{CompletionRegistryError, ReclaimStatus};

use super::{
    AlterConsumerGroupOffsetsHost, AlterConsumerGroupOffsetsHostError, response::terminal_input,
};

impl AlterConsumerGroupOffsetsHost {
    pub(super) fn poll_one_call(&mut self) -> Result<bool, AlterConsumerGroupOffsetsHostError> {
        let Some(index) = self
            .operations
            .iter()
        ...
        match terminal {
            ...
            Err(_error) => Err(AlterConsumerGroupOffsetsHostError::CallCompletion),
        }
    }
    ...
    pub(super) fn settle_raw(
        ...
        let input = {
            ...
            let raw = operation
                .raw_terminal
                .as_ref()
                .ok_or(AlterConsumerGroupOffsetsHostError::MissingTerminal)?;
        ...
        match transition.into_effect() {
            ...
            _ => Err(AlterConsumerGroupOffsetsHostError::MissingTerminal),
        }
    }
    ...
    pub(super) fn publish_terminal(
        ...
        match self.completions.publish(completion_id, terminal) {
            ...
            Err((error, terminal)) => {
                self.operations[index].terminal = Some(terminal);
                Err(AlterConsumerGroupOffsetsHostError::Completion(error))
            }
        }
    }

    pub(super) fn reclaim_one(&mut self) -> Result<bool, AlterConsumerGroupOffsetsHostError> {
        ...
        match self.completions.finish_reclaim(completion_id) {
            ...
            Err(error) => Err(AlterConsumerGroupOffsetsHostError::Completion(error)),
        }
    }
    ...
    fn release_published_bytes(
        ...
        let index = self
            ...
            .ok_or(AlterConsumerGroupOffsetsHostError::ByteAccounting)?;
