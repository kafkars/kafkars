//! Stable generated-free terminals for Admin `AddRaftVoter`.
...
pub enum AddRaftVoterDeliveryStatus {
    /// The destructive call did not reach Kafka transport ownership.
    NotSent,
...
pub struct AddRaftVoterFailure {
    pub(super) kind: AddRaftVoterFailureKind,
    pub(super) delivery: AddRaftVoterDeliveryStatus,
}
...
impl AddRaftVoterFailure {
    ...
    pub const fn delivery(self) -> AddRaftVoterDeliveryStatus {
        self.delivery
    }
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum AddRaftVoterOutcome {
    /// Kafka committed the voter addition.
    Added(AddRaftVoterResult),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum AddRaftVoterObserverError {
...
impl fmt::Display for AddRaftVoterObserverError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self {
            Self::AlreadyObserved => "Admin AddRaftVoter result was already observed",
            Self::Stale => "Admin AddRaftVoter observer is stale",
        })
    }
}

impl std::error::Error for AddRaftVoterObserverError {}
