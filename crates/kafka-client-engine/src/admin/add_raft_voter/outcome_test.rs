...
use kafka_client_core::{
    AddRaftVoterBrokerError as CoreBrokerError, AddRaftVoterSuccess, AddRaftVoterTerminal,
};

use super::{AddRaftVoterOutcome, outcome::translate_terminal};

#[test]
fn successful_result_exposes_throttle_as_scalar_part() {
    let AddRaftVoterOutcome::Added(result) =
        translate_terminal(AddRaftVoterTerminal::Added(AddRaftVoterSuccess::new(17)))
    else {
...
fn broker_error_preserves_signed_code_and_bounded_message() {
    ...
    let AddRaftVoterOutcome::BrokerRejected(error) =
        translate_terminal(AddRaftVoterTerminal::BrokerRejected(core))
    else {
