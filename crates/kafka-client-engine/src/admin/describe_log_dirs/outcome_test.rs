...
fn translation_retains_nested_codes_version_presence_and_replica_scalars() {
    ...
    let (broker, result) = outcomes
        .into_iter()
        .next()
        .unwrap_or_else(|| panic!("first broker"))
        .into_parts();
    assert_eq!(broker, 7);
    let DescribeLogDirsEngineBrokerResult::Described(log_dirs) = result else {
        panic!("expected directories");
    };
    let (path, description) = log_dirs
        .into_iter()
        .next()
        .unwrap_or_else(|| panic!("directory"))
        .into_parts();
    assert_eq!(path, "/data");
    let (replicas, total, usable, cordoned) = description
        .unwrap_or_else(|error| panic!("successful directory: {error:?}"))
        .into_parts();
    assert_eq!(
