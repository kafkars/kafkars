//! Lossless engine translation scenarios for `DescribeTopics` terminals.
...
fn topic_and_partition_codes_cross_the_engine_boundary_exactly() {
    ...
    let terminal = DescribeTopicsTerminal::Topics(vec![
        ...
        DescribeTopicOutcome::failed("audit", DescribeTopicBrokerError::new(topic_code)),
    ]);
    let DescribeTopicsOutcome::Topics(topics) = translate_terminal(terminal) else {
        panic!("topic terminal expected");
    };
    let (_, orders) = topics[0].clone().into_parts();
    ...
    assert_eq!(isr, [7]);
    assert_eq!(offline, [8]);
    let (_, audit) = topics[1].clone().into_parts();
