//! API-key 75 publication and closed-capacity evidence for the shared notifier.

use kafka_client_core::{DescribeTopicPartitionsPage, DescribeTopicPartitionsTerminal};
...
#[test]
fn shared_worker_publishes_describe_topic_partitions_off_reactor() {
    ...
    let page = DescribeTopicPartitionsPage::new(17, Vec::new(), None)
        .unwrap_or_else(|error| panic!("valid exact API 75 page: {error}"));
    exercise_terminal(
        ports.describe_topic_partitions,
        DescribeTopicPartitionsTerminal::Page(page),
        worker,
    );
    let join = notifier
        .stop()
...
pub(super) fn assert_shared_capacity_includes_describe_topic_partitions() {
    assert_eq!(
