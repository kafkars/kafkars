...
use super::retention::{RESULT_DIAGNOSTIC_BYTES_PER_TOPIC, request_charge, result_fixed_charge};

#[test]
fn request_reservation_covers_fixed_results_and_explicit_diagnostics() {
    let topic_bytes = "orders".len();
    let request =
        request_charge(1, 0, topic_bytes).unwrap_or_else(|| panic!("small request charge fits"));
