...
use crate::{
    ...
    consumer::{AssignedConsumerFaultKind, AssignedPartitionInput},
    driver::DriverOwner,
};
...
fn contention_is_blocked_without_false_progress() {
    let (mut driver, clock, shard, _port) = setup();
...
fn each_host_call_runs_one_bounded_owner_turn() {
    let (mut driver, clock, shard, port) = setup();
    let _accepted = port
        .replace_assignment(
...
fn shutdown_waits_for_core_authorized_close_retention() {
    ...
    for _attempt in 0..8 {
        ...
        completed = progress.close_completed;
        if completed {
...
fn setup() -> (
    ...
    let (owner, port) =
        start_assigned_consumer(Arc::clone(&clock), Arc::new(driver.reactor_wake()))
            .unwrap_or_else(|error| panic!("assigned consumer: {error:?}"));
    (driver, clock, owner, port)
}
