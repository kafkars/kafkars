//! Exact API 38 ownership, capability, test-evidence, and protected-call ratchets.

#[path = "admin_create_delegation_token_guard/expectations.rs"]
mod expectations;
mod support;
...
#[test]
fn checked_in_api_38_policy_is_exact() {
    ...
    let allows = admin
        .allow
        .iter()
        .filter(|allow| allow.path.contains("/create_delegation_token/"))
...
fn fixture_rejects_cloneable_and_foreignly_mutated_owners() {
    let (root, files) = fixture_files("admin_create_delegation_token");
    let linear = LINEAR
        .iter()
...
fn fixture_rejects_second_submission_and_deadline_capture_owners() {
    let (root, _) = fixture_files("admin_create_delegation_token");
    let rules = [
        MethodCapabilityRule {
            root: "src".to_owned(),
            method: "submit_tracked_create_delegation_token".to_owned(),
            allowed_paths: vec!["src/method_owner.rs".to_owned()],
        },
        MethodCapabilityRule {
            root: "src".to_owned(),
            method: "capture_create_delegation_token".to_owned(),
            allowed_paths: vec!["src/method_owner.rs".to_owned()],
        },
    ];
