//! Deliberately reaches policy and hidden execution from the notifier.
...
async fn violate() {
    ...
    let _future = future::pending::<()>();
    let _net = net;
    let _thread = std::thread::spawn(|| {});
    let _instant = time::Instant::now();
    let _system = time::SystemTime::now();
    let _callback = Callback;
    let _future_trait = Future;
