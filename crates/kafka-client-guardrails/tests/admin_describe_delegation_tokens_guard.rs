//! Exact API 41 ownership, capability, test-evidence, and protected-call ratchets.

#[path = "admin_describe_delegation_tokens_guard/expectations.rs"]
mod expectations;
mod support;
...
#[test]
fn checked_in_api_41_policy_is_exact() {
    ...
    let allows = admin
        .allow
        .iter()
        .filter(|allow| allow.path.contains("/describe_delegation_tokens/"))
...
fn fixture_rejects_cloneable_and_foreignly_mutated_owners() {
    let (root, files) = fixture_files("admin_describe_delegation_tokens");
    let linear = LINEAR
        .iter()
...
fn fixture_rejects_second_submission_and_deadline_capture_owners() {
    let (root, _) = fixture_files("admin_describe_delegation_tokens");
    let rules = [
        MethodCapabilityRule {
            root: "src".to_owned(),
            method: "submit_tracked_describe_delegation_tokens".to_owned(),
            allowed_paths: vec!["src/method_owner.rs".to_owned()],
        },
        MethodCapabilityRule {
            root: "src".to_owned(),
            method: "capture_describe_delegation_tokens".to_owned(),
            allowed_paths: vec!["src/method_owner.rs".to_owned()],
        },
    ];
