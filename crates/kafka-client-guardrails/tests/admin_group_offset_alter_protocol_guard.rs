//! Negative evidence for offset-alteration protocol and call ownership boundaries.
...
const CALL: &str = "crates/kafka-client-engine/src/driver/rpc/group_offset_alter_call.rs";
const TERMINAL: &str = "crates/kafka-client-engine/src/driver/rpc/group_offset_alter_terminal.rs";
const LINEAR: &[(&str, &str)] = &[
    ("GroupOffsetAlterCall", CALL),
    ("GroupOffsetAlterCallAdmissionFailure", CALL),
    ("GroupOffsetAlterTerminal", TERMINAL),
    ("RecoveredGroupOffsetAlterCall", TERMINAL),
];
...
fn checked_in_linear_and_submission_capability_policy_is_exact() {
    ...
    let rules = config
        .method_capabilities
        .iter()
        .filter(|rule| rule.method == "submit_tracked_group_offset_alter")
        .collect::<Vec<_>>();
...
fn fixture_rejects_clone_and_copy_for_every_linear_owner() {
    let (root, files) = fixture_files("admin_group_offset_alter_protocol");
    let rules = LINEAR
        .iter()
...
fn fixture_rejects_a_second_direct_driver_submission_owner() {
    let (root, _) = fixture_files("admin_group_offset_alter_protocol");
    let rule = MethodCapabilityRule {
        root: "src".to_owned(),
        method: "submit_tracked_group_offset_alter".to_owned(),
    ...
    assert!(violations.iter().any(|violation| {
        violation.contains("submission_intruder.rs")
            && violation.contains("submit_tracked_group_offset_alter")
    }));
    assert!(
        !violations
            .iter()
