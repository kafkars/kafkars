...
use super::{
    AdminConsumerGroupListing, AdminGroupListingFilters, AdminGroupListingScope,
    AdminListConsumerGroupsBrokerError, AdminListConsumerGroupsBrokerOutcome,
...
fn discovery_then_exact_brokers_filters_sorts_and_deduplicates_consumer_groups() {
    let mut machine = machine(AdminGroupListingScope::ConsumerOnly);
    assert_eq!(machine.scope(), AdminGroupListingScope::ConsumerOnly);
    let first = effect(
        &mut machine,
        ...
        AdminListConsumerGroupsInput::BrokerResponded {
            ...
            outcome: AdminListConsumerGroupsBrokerOutcome::Groups {
                ...
                groups: vec![
                    ...
                    listing("shared", "", Some("Stable"), Some("share")),
                    listing("streams", "", Some("Stable"), Some("streams")),
...
#[test]
fn all_scope_retains_mixed_group_types_then_sorts_and_deduplicates_globally() {
    let mut machine = machine(AdminGroupListingScope::All);
    assert_eq!(machine.scope(), AdminGroupListingScope::All);
    let _ = effect(
        &mut machine,
        AdminListConsumerGroupsInput::Start {
            now: Moment::from_tick(1),
    ...
    let second_broker = effect(
        ...
        AdminListConsumerGroupsInput::BrokerResponded {
            ...
            outcome: AdminListConsumerGroupsBrokerOutcome::Groups {
                broker_id: 3,
                groups: vec![
                    listing("classic", "", Some("Empty"), Some("classic")),
                    listing("share", "", Some("Stable"), Some("share")),
                    listing("same", "", Some("Stable"), Some("streams")),
                    listing("zeta", "consumer", Some("Stable"), Some("classic")),
                ],
            },
        },
    ...
    let terminal = effect(
        ...
        AdminListConsumerGroupsInput::BrokerResponded {
            ...
            outcome: AdminListConsumerGroupsBrokerOutcome::Groups {
                broker_id: 8,
                groups: vec![
                    listing("consumer", "", Some("Stable"), Some("consumer")),
                    listing("streams", "", Some("Stable"), Some("streams")),
                    listing("same", "", Some("Empty"), Some("consumer")),
                ],
            },
        },
    );
    ...
    assert_eq!(groups[2].group_type(), Some("consumer"));
    assert_eq!(groups[2].group_state(), Some("Empty"));
}
...
#[test]
fn protocol_type_filters_are_applied_exactly_after_broker_normalization() {
    ...
    let mut machine = AdminListConsumerGroupsMachine::new(
    ...
    machine
        .apply(AdminListConsumerGroupsInput::DriverAccepted)
        .unwrap_or_else(|error| panic!("accept discovery: {error}"));
    let submission = effect(
        &mut machine,
        AdminListConsumerGroupsInput::BrokersDiscovered {
            broker_ids: vec![3],
        },
    );
    let AdminListConsumerGroupsEffect::SubmitBroker {
        filters: submitted, ..
    } = submission
    else {
        panic!("expected broker submission");
    };
    assert_eq!(submitted, filters);
    machine
        .apply(AdminListConsumerGroupsInput::DriverAccepted)
        .unwrap_or_else(|error| panic!("accept broker: {error}"));
    let terminal = effect(
        ...
        AdminListConsumerGroupsInput::BrokerResponded {
            ...
            outcome: AdminListConsumerGroupsBrokerOutcome::Groups {
                broker_id: 3,
                groups: vec![
                    listing("kept", "consumer", Some("Stable"), Some("classic")),
                    listing("dropped", "", Some("Stable"), Some("classic")),
                ],
            },
        },
    );
    ...
    let (_throttle, groups, errors) = batch.into_parts();
    assert!(errors.is_empty());
    assert_eq!(groups[0].group_id(), "kept");
    assert_eq!(groups.len(), 1);
}

fn machine(scope: AdminGroupListingScope) -> AdminListConsumerGroupsMachine {
    AdminListConsumerGroupsMachine::new(
        OperationId::from_raw(5),
        Deadline::from_tick(100),
        scope,
