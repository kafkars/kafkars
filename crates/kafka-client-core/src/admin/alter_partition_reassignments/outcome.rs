//! Protocol-normalized terminal values for reassignment alteration.
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AlterPartitionReassignmentResult {
    /// Kafka accepted the replacement or cancellation.
    Altered,
    /// Kafka rejected this specific change.
    Failed(AlterPartitionReassignmentBrokerError),
}

/// One per-partition result retained in original caller order.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct AlterPartitionReassignmentOutcome {
    topic: String,
    partition: i32,
    result: AlterPartitionReassignmentResult,
}

impl AlterPartitionReassignmentOutcome {
    /// Creates one successful change result.
    pub const fn altered(topic: String, partition: i32) -> Self {
        Self {
            topic,
            partition,
            result: AlterPartitionReassignmentResult::Altered,
        }
    }
    ...
    pub const fn failed(
        ...
        Self {
            topic,
            partition,
            result: AlterPartitionReassignmentResult::Failed(error),
        }
    }
    ...
    pub const fn result(&self) -> &AlterPartitionReassignmentResult {
        &self.result
    }

    /// Consumes this outcome into adapter-owned scalar values.
    pub fn into_parts(self) -> (String, i32, AlterPartitionReassignmentResult) {
...
pub struct AlterPartitionReassignmentsBatch {
    ...
    outcomes: Vec<AlterPartitionReassignmentOutcome>,
}
...
impl AlterPartitionReassignmentsBatch {
    ...
    pub fn outcomes(&self) -> &[AlterPartitionReassignmentOutcome] {
        &self.outcomes
    }

    /// Consumes the batch into adapter-owned parts.
    pub fn into_parts(self) -> (u32, Vec<AlterPartitionReassignmentOutcome>) {
...
impl AlterPartitionReassignmentsFailure {
    ...
    pub fn into_parts(self) -> (AlterPartitionReassignmentsFailureKind, DeliveryStatus) {
        (self.kind, self.delivery)
    }
}

/// Exactly one terminal decision for reassignment alteration.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AlterPartitionReassignmentsTerminal {
