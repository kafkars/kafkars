//! Success, exact broker rejection, and mechanism terminals for `RemoveRaftVoter`.
...
pub const REMOVE_RAFT_VOTER_DIAGNOSTIC_BYTES: usize = 1024;

/// Successful voter removal and Kafka's throttle observation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct RemoveRaftVoterSuccess {
    throttle_time_ms: u32,
}

impl RemoveRaftVoterSuccess {
...
/// Exact top-level API-81 broker rejection.
...
/// Whole-operation failure outside an exact API-81 broker rejection.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl RemoveRaftVoterFailure {
    pub(crate) const fn new(kind: RemoveRaftVoterFailureKind, delivery: DeliveryStatus) -> Self {
        Self { kind, delivery }
    }
...
/// Exactly one terminal decision for Admin `RemoveRaftVoter`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum RemoveRaftVoterTerminal {
    /// Kafka accepted the voter removal.
    Removed(RemoveRaftVoterSuccess),
