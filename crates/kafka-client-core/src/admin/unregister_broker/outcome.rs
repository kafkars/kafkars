//! Successful, broker-rejected, and mechanism terminal facts for `UnregisterBroker`.
...
pub const UNREGISTER_BROKER_DIAGNOSTIC_BYTES: usize = 1024;

/// Successful broker unregistration and Kafka's throttle observation.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct UnregisterBrokerSuccess {
    throttle_time_ms: u32,
}

impl UnregisterBrokerSuccess {
...
/// Exact top-level API-64 broker rejection.
...
/// Whole-operation failure outside an exact API-64 broker rejection.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl UnregisterBrokerFailure {
    pub(crate) const fn new(kind: UnregisterBrokerFailureKind, delivery: DeliveryStatus) -> Self {
        Self { kind, delivery }
    }
...
/// Exactly one terminal decision for Admin `UnregisterBroker`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum UnregisterBrokerTerminal {
    /// Kafka accepted the destructive broker unregistration.
    Unregistered(UnregisterBrokerSuccess),
