//! Ordered terminal facts for one resource-generic `LegacyAlterConfigs` operation.
...
pub struct LegacyAlterConfigBrokerError {
    code: NonZeroI16,
    message: Option<String>,
    message_truncated: bool,
}

impl LegacyAlterConfigBrokerError {
...
/// Per-resource legacy alteration result.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LegacyAlterConfigResult {
    /// Kafka accepted every requested alteration for this topic.
    Altered,
    /// Kafka rejected this topic with an exact signed code.
    Failed(LegacyAlterConfigBrokerError),
}

/// One resource result retained in original request order.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LegacyAlterConfigOutcome {
    resource_type: i8,
    resource_name: String,
    result: LegacyAlterConfigResult,
}

impl LegacyAlterConfigOutcome {
    /// Creates one successful topic result.
    pub fn altered(topic: impl Into<String>) -> Self {
        Self {
            resource_type: 2,
            resource_name: topic.into(),
            result: LegacyAlterConfigResult::Altered,
        }
    }

    /// Creates one broker-rejected topic result.
    pub fn failed(topic: impl Into<String>, error: LegacyAlterConfigBrokerError) -> Self {
        Self {
            resource_type: 2,
            resource_name: topic.into(),
            result: LegacyAlterConfigResult::Failed(error),
        }
    }
    ...
    /// Creates one broker-rejected result for an exact requested resource.
    pub fn resource_failed(
        resource_type: i8,
        resource_name: impl Into<String>,
        error: LegacyAlterConfigBrokerError,
    ) -> Self {
        Self {
            resource_type,
            resource_name: resource_name.into(),
    ...
    pub const fn result(&self) -> &LegacyAlterConfigResult {
        &self.result
    }

    /// Consumes this outcome into adapter-owned parts.
    pub fn into_parts(self) -> (String, LegacyAlterConfigResult) {
        (self.resource_name, self.result)
    }

    /// Consumes this outcome into its exact resource identity and result.
    pub fn into_resource_parts(self) -> (i8, String, LegacyAlterConfigResult) {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LegacyAlterConfigsBatch {
    throttle_time_ms: u32,
    resources: Vec<LegacyAlterConfigOutcome>,
}

impl LegacyAlterConfigsBatch {
    /// Creates one protocol-normalized ordered response batch.
    pub const fn new(throttle_time_ms: u32, resources: Vec<LegacyAlterConfigOutcome>) -> Self {
        Self {
            throttle_time_ms,
            resources,
        }
    ...
    pub fn resources(&self) -> &[LegacyAlterConfigOutcome] {
        &self.resources
    }

    /// Returns topic-compatible outcomes in original request order.
    pub fn topics(&self) -> &[LegacyAlterConfigOutcome] {
        &self.resources
    }

    /// Consumes the batch into adapter-owned parts.
    pub fn into_parts(self) -> (u32, Vec<LegacyAlterConfigOutcome>) {
...
pub enum LegacyAlterConfigsFailureKind {
    ...
    ResponseTooLarge,
    /// The broker cannot execute the requested legacy full-snapshot semantics.
    Compatibility,
}
...
impl LegacyAlterConfigsFailure {
    pub(crate) const fn new(kind: LegacyAlterConfigsFailureKind, delivery: DeliveryStatus) -> Self {
        Self { kind, delivery }
    }
...
/// Exactly one terminal decision for a legacy full-snapshot configuration operation.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum LegacyAlterConfigsTerminal {
    /// Ordered per-topic outcomes and retained throttle observation.
    Configs(LegacyAlterConfigsBatch),
